...
- **Coder C**: C04 (Recovery matrix integration, Graph-provider engine)

At Gate P4, checked simulators are replaced with real reviewed package versions,
the frontend contract is frozen, and the integrated live settlement proof is
executed. Graph-provider/model promotion remains a separate fresh-trace gate;
the Graph recovery path is integrated and fail-closed. Gate P4 is PASS for the
converged backend and settlement proof.
...
| Graph Recovery              | `@oneshot/reconciliation`                              | Lane C          | Integrated; fresh provider/model trace pending                                             |
...
5. **Prove live hashless recovery**: [COMPLETED]
   ...
   - Queried it through the live Subgraph Studio endpoint using native `STUDIO_GRAPHQL` transport; no MCP label is emitted.
   ...
   - Historical proof is retained, but The Graph remains `NOT VERIFIED` until a fresh Studio trace shows meaningful agent/core use; the deterministic safety core remains authoritative.
