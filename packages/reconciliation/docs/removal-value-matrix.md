...
| OneShot/Arc Subgraph through Subgraph MCP | Structured transfer tuple/window candidates for an LLM recovery agent | `_meta` deployment, indexed block/time, indexing errors, and RPC head lag | Must be proven live | The Graph Gateway plus MCP | High for recovery views and agent explanations | Required live path for the selected AI track |
