...
test('policy denial and committed settlement render through B05', async ({ page }) => {
  await selectIntent(page, 'intent-denied', 'Settlement evidence');
  await expect(page.getByText('Denied', { exact: true })).toBeVisible();
  await selectIntent(page, 'intent-committed', 'Settlement evidence');
...
test('service-unavailable, keyboard, responsive, and token-memory checks fail safe', async ({
  ...
  await page.getByRole('tab', { name: 'Create or replay' }).focus();
  await page.keyboard.press('ArrowRight');
  await expect(page.getByRole('tab', { name: 'Authoritative status' })).toHaveAttribute(
    'aria-selected',
    'true',
  );
  await expect(page.getByRole('tab', { name: 'Authoritative status' })).toBeFocused();
});
